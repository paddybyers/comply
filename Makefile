...
comply: assets $(GO_SOURCES)
	...
	@# $(eval LDFLAGS := -ldflags='-X "github.com/paddybyers/comply/internal/cli.Version=$(VERSION)"')
	go build $(LDFLAGS) github.com/paddybyers/comply
...
install: assets $(GO_SOURCES)
	go install github.com/paddybyers/comply
...
release: release-env dist release-deps
	...
	$(eval SHA := $(shell curl -s -L https://github.com/paddybyers/comply/archive/$(VERSION).tar.gz |shasum -a 256|cut -d" " -f1))
...
docker-release:
	docker build --build-arg COMPLY_VERSION=`cat VERSION` -t paddybyers/comply .
	docker push paddybyers/comply
